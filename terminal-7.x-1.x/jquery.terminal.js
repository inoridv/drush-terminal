      ...
      terminal_form.submit(function() {
        return false;
			...
			var terminal_append = function(data) {
				...
				data = '';
				dataRows.pop();
				// Remove o timelapse da execução do comando no fim do output
					for (row in dataRows) {
						if (dataRows[row] == "[Error]" || dataRows[row] == "[Warning]") {
							data += '<span style="color:red">' + dataRows[row].replace(/\t/g, tabString);
							// Verificação Hardcoded para destacar as mensagens de erro e de warnings (Temporário).
						}
						else {
